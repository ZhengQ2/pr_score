...
// Question order is the questionnaire order. `multi` questions take an array of values.
export const questions=[
 ...
 {id:'destination',title:'Where would you like to live in Canada?',hint:'Provincial programs require you to intend to live in that province. Express Entry is for living outside Quebec.',options:[['any','Anywhere: I’m flexible'],...PROVINCES]},
...
export const optionLabel=(q,value)=>q.multi?(value?.length?value.map(v=>optionLabel({options:q.options},v)).join(', '):'None'):(q.options.find(o=>String(o[0])===String(value))?.[1]??'Not answered');
...
function facts(a){
 ...
 const f={en:n('english'),fr:n('french'),canada:n('canadaYears'),foreign:n('foreignYears'),age:AGES[a.age]??30,education:a.education??'secondary',
  ...
  inCanada:a.location==='canada',destination:a.destination??'any',funds:tri(a.funds),spouse:a.spouse==='yes',family:a.family??[],history:a.history??[]};
...
const livesIn=(f,code)=>[`Plan to live in ${regionName(code)}`,f.destination==='any'||f.destination===code];
...
function expressEntry(f){
 ...
 const outside=['Plan to live outside Quebec',f.destination!=='qc'];
...
function unscoredMatch(f,code){
 const relevant=f.offer===code||f.destination===code||f.family.includes(code)||f.history.includes(code);
 if(!relevant)return null;
 if(code==='qc')return {paths:[{name:'Quebec skilled worker selection',checks:[['Plan to live in Quebec',f.destination==='any'||f.destination==='qc'],['Quebec selection criteria, including French',null]]}],notes:['Quebec selects its own skilled workers; you apply for federal permanent residence after receiving a Quebec Selection Certificate.']};
...
export function recommend(answers){
 ...
 const add=(program,rules,code,external)=>{const path=bestPath(rules.paths);results.push({id:program.slug??code,code,region:program.region,name:program.name,detail:program.detail??program.note,href:external?program.url:`/${program.slug}`,external:Boolean(external),fit:path.fit,met:path.met,path:path.name,checks:path.checks,notes:rules.notes??[],estimate:rules.estimate??null,preferred:f.destination===code});};
